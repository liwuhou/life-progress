...
The system SHALL use `~/.config/live_progress/profile.toml` as the sole canonical location for a shared user profile. It SHALL not treat the legacy `~/.life_progress/config.toml` path as initialized profile data.

#### Scenario: Loading a configured profile

- **WHEN** `~/.config/live_progress/profile.toml` exists and is valid
